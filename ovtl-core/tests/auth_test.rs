...
async fn test_me_endpoint_logic() {
    ...
    let decrypted = hefesto::decrypt(&fetched.email, &tenant_key, &cfg.master_encryption_key)
        .unwrap();
...
async fn test_refresh_token_rotation() {
    ...
    let user = create_test_user(&db, &cfg, tenant_id, &tenant_key, "refresh@ovtl.dev", "Pass1234!").await;
...
async fn test_revoke_all_tokens() {
    ...
    let user = create_test_user(&db, &cfg, tenant_id, &tenant_key, "revoke@ovtl.dev", "Pass1234!").await;
    ...
    let txn = db::begin_tenant_txn(&db, tenant_id).await.unwrap();
    token_service::revoke_all_user_tokens(&txn, user.id).await.unwrap();
    txn.commit().await.unwrap();
...
async fn test_lockout_and_clear() {
    let (db, cfg, tenant_id, tenant_key) = setup().await;
    let email = "lockout_test@ovtl.dev";
    let lookup = hefesto::hash_for_lookup(email, &tenant_key).expect("hash");

    lockout_service::clear_attempts(&db, tenant_id, &lookup).await.unwrap();

    // Not locked initially
    assert!(!lockout_service::is_locked(&db, tenant_id, &lookup, 3, 15).await.unwrap());

    // Record 3 attempts
    for _ in 0..3 {
        lockout_service::record_attempt(&db, tenant_id, &lookup).await.unwrap();
    }
...
async fn test_one_time_token_consume() {
    let (db, cfg, tenant_id, tenant_key) = setup().await;
    let user = create_test_user(&db, &cfg, tenant_id, &tenant_key, "ott@ovtl.dev", "Pass1234!").await;
    ...
    one_time_token_service::store(
        ...
        user.id,
        hash,
        ovtl_core::entity::one_time_tokens::TYPE_PASSWORD_RESET,
        60,
    )
    .await
    .unwrap();
...
async fn test_otp_email_verification() {
    let (db, cfg, tenant_id, tenant_key) = setup().await;
    let user = create_test_user(&db, &cfg, tenant_id, &tenant_key, "otp_verify@ovtl.dev", "Pass1234!").await;

    let otp = one_time_token_service::generate_otp();
    assert_eq!(otp.len(), 6);
    assert!(otp.chars().all(|c| c.is_ascii_digit()));

    one_time_token_service::store_otp(&db, tenant_id, user.id, &otp, 1).await.unwrap();
...
async fn test_totp_verify_correct_code() {
    ...
    let now = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_secs();
    let code = totp_lite::totp_custom::<sha1::Sha1>(
    ...
    assert!(mfa_service::verify_code(&secret, &code), "current code must verify");
}
...
async fn test_mfa_token_roundtrip() {
    ...
    let claims = token_service::verify_mfa_token(&token, &cfg.jwt_secret)
        .expect("verify mfa token");
...
async fn test_mfa_totp_uri_format() {
    let secret = mfa_service::generate_secret();
    let uri = mfa_service::totp_uri(&secret, "user@example.com", "OVTL");
